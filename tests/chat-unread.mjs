...
assert.deepEqual(await get(),{status:200,unread:2,read:chatRead(null)},'Exclude own messages and other towns/channels');
assert.deepEqual(await get('mow'),{status:200,unread:1,read:chatRead(null)});
...
async function save(read,channel='town',townId='town-a',origin='https://townies.test'){
 const response=await POST(new Request('https://townies.test/api/chat',{method:'POST',headers:{origin,'content-type':'application/json'},body:JSON.stringify({action:'read',read,channel,townId,resident:'neighbor'})}));
 return {status:response.status,...await response.json()};
}
assert.equal((await save(cursor)).changed,true);
assert.equal((await get()).unread,1,'Fresh device with no local cursor uses saved account progress');
assert.equal((await save(read)).changed,true);
assert.equal((await get()).unread,0,'Both messages at the boundary are remembered');
assert.equal((await save(cursor)).changed,false,'An older device cannot erase acknowledged boundary IDs');
assert.equal((await save(readMessages([{id:'old-message-00001',created:50}]))).changed,false,'Older timestamps cannot rewind progress');
assert.equal((await get()).unread,0);
message('new-message-0001','town',120);
assert.equal((await get()).unread,1,'Only later messages are unread after a new login');
assert.equal((await get('mow')).unread,1,'Reading town chat does not clear profession chat');
assert.equal((await save(readMessages([{id:'profession-00001',created:100}]),'mow')).status,200);
assert.equal((await get('mow')).unread,0);
assert.equal((await get()).unread,1);
assert.equal((await save({created:Date.now()+100000,ids:['new-message-0001']})).status,400,'Future timestamps cannot suppress future messages');
assert.equal((await save({created:120,ids:['unknown-message-0001']})).status,400);
assert.equal((await save({created:100,ids:['other-town-00001']})).status,400,'Cannot use another town boundary');
assert.equal((await save(read,'garden')).status,403);
assert.equal((await save(read,'town','town-b')).status,409);
assert.equal((await save(read,'town','town-a','https://other.test')).status,403);
const otherHash=Buffer.from(await crypto.subtle.digest('SHA-256',new TextEncoder().encode('account-b'))).toString('hex');
resident.run('reader-b','town-a','Reader B','mow',2,otherHash);
globalThis.chatTestIdentity='account-b';
assert.equal((await get()).unread,5,'Another account has independent progress, including the first account’s messages');
assert.equal((await save(read)).changed,true);
globalThis.chatTestIdentity='account-a';
assert.equal((await get()).unread,1,'Another account cannot change this account’s cursor');
assert.equal(database.prepare("SELECT COUNT(*) AS n FROM chat_read_positions WHERE resident_id='neighbor'").get().n,0,'Payload cannot select another resident');
...
globalThis.chatTestIdentity=null;assert.equal((await get()).status,401);assert.equal((await save(read)).status,401);
database.close();delete globalThis.chatTestDb;delete globalThis.chatTestIdentity;
console.log('PASS: persistent cross-device read progress, new-login counts, own-message exclusion, channel/town/account authorization, same-time arrivals, atomic monotonic merging, and moderation.');
