...
for(let i=0;i<2;i++){const d=await api(i,'setup',{home:i,job:'mow'});assert.match(d.resident.id,/^[0-9a-f-]{36}$/);execFileSync(process.execPath,['node_modules/wrangler/bin/wrangler.js','d1','execute','DB','--local','--config','wrangler.local.json','--command',`UPDATE residents SET x=-36.6,z=-9.3 WHERE id='${d.resident.id}'`],{stdio:'pipe'});}
await pause(1200);
for(let i=0;i<2;i++){assert.equal((await api(i,'heartbeat',{x:-36.6,z:-9.3})).corrected,false);await api(i,'mower',{active:true})}
assert.ok((await api(0)).peers.every(p=>p.mowing));
assert.equal((await api(0,'heartbeat',{x:-36.6,z:-9.3})).mowReward,null,'A parked mower earns nothing');
await pause(1100);
const race=await Promise.all([api(0,'heartbeat',{x:-31.5,z:-9.3}),api(1,'heartbeat',{x:-31.5,z:-9.3})]);
...
await pause(1100);
const repeat=await api(0,'heartbeat',{x:-36.6,z:-9.3});assert.equal(repeat.mowReward,null,'Cut stripes cannot pay again');
await api(0,'mower',{active:false});await pause(1100);
const walking=await api(0,'heartbeat',{x:-31.5,z:-8});assert.equal(walking.mowReward,null,'Walking does not cut grass');
...
await pause(1100);
const concurrent=await Promise.all([api(0,'heartbeat',{x:-36.6,z:-8}),api(0,'heartbeat',{x:-36.6,z:-8})]);
...
const helper=await api(0,'heartbeat',{x:-31.5,z:-6.7});assert.ok(helper.mowReward?.patches>0);assert.equal(helper.mowReward.coins,helper.mowReward.patches,'Non-career mowing pays one coin per patch');
await pause(4200);
const stale=await api(0,'heartbeat',{x:-36.6,z:-6.7});assert.equal(stale.mowReward,null,'Offline gaps cannot sweep unsent paths for money');
...
await api(0,'heartbeat',{x:-36.6,z:-6.7});await pause(1100);
const recut=await api(0,'heartbeat',{x:-31.5,z:-6.7});assert.ok(recut.mowReward?.patches>0,'Regrown grass can be cut for income again');
