...
mode='pan';send('pointerdown',0,0,1);send('pointerdown',100,0,2);send('pointermove',200,0,2);assert.ok(calls.zoom.at(-1)[0]<0,'Spreading fingers zooms in');send('pointermove',0,200,2);close(calls.orbit.at(-1)[0],Math.PI/2);send('pointermove',200,0,2);close(calls.orbit.at(-1)[0],-Math.PI/2);send('pointerup',0,0,1);send('pointerup',0,200,2);assert.equal(calls.tap.length,1,'Two-finger gestures never trigger walking');
...
console.log('PASS: camera-relative movement at every angle, bounded tilt, drag/tap separation, right-drag, clockwise/counterclockwise touch rotation, pinch zoom, cancellation, and cleanup.');
