...
assert.equal(HOME_LOTS.length,50);assert.equal(new Set(HOME_LOTS.map(p=>`${p.x}:${p.z}`)).size,50);
for(const [i,h]of HOME_LOTS.entries()){
 assert.ok(!BUILDINGS.some(b=>Math.abs(h.x-b.x)<b.width/2+3&&Math.abs(h.z-b.z)<b.depth/2+3));
...
for(const bridge of BRIDGES){
 for(const direction of [1,-1]){
  const start={x:direction===1?20:36,z:bridge.z},goal={x:direction===1?36:20,z:bridge.z};const path=findPath(start,goal,isTownBlocked);assert.ok(path.length);assert.deepEqual(path.at(-1),goal);assert.ok(path.every(p=>!isTownBlocked(p.x,p.z)));assert.ok(path.some(p=>p.x===28));
 }
 for(let x=21;x<=35;x+=.1){assert.ok(!isTownBlocked(x,bridge.z));assert.ok(groundHeight(x,bridge.z)>=0&&groundHeight(x,bridge.z)<=.4)}
