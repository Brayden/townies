...
assert.ok(
  LAWN_CELLS.every((c) => !isTownBlocked(c.x, c.z)),
  'No grass inside buildings or water',
);
...
for (const [x, z] of [
  ...
  assert.ok(
    town.some((c) => Math.hypot(c.x - x, c.z - z) < 4),
    `Grass should grow near ${x},${z}`,
  );
// Compare the spatial lookup with the complete geometric answer across varied routes.
for (let i = 0; i < 150; i++) {
  const ax = -58 + ((i * 17.31) % 116),
    az = -52 + ((i * 9.27) % 102),
    bx = ax + Math.sin(i) * 6,
    bz = az + Math.cos(i) * 6;
  const dx = bx - ax,
    dz = bz - az,
    length = dx * dx + dz * dz;
  const expected = LAWN_CELLS.filter((c) => {
    const t = Math.max(
      0,
      Math.min(1, ((c.x - ax) * dx + (c.z - az) * dz) / length),
    );
    return Math.hypot(c.x - ax - t * dx, c.z - az - t * dz) < 0.62;
  })
    .map((c) => c.id)
    .sort();
  assert.deepEqual(
    sweptGrass(ax, az, bx, bz)
      .map((c) => c.id)
      .sort(),
    expected,
  );
...
console.log(
  `PASS: ${town.length} mowable town grass patches, coverage across all neighborhoods, protected roads and buildings, stable original IDs, and accurate spatial cutting queries.`,
);
