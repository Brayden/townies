...
### Construction grid and explicit branching

Planning now uses vote → fund → place for public buildings. Full funding changes a branch, relocation, or redevelopment to `ready`; the old facility remains in place. The elected mayor then commits an integer grid position and a 0/90/180/270-degree facing. Placement spends no additional money, closes the plan atomically with its world change and ledger entry, and rejects repeated submissions. Expansion acquisitions still open land immediately at full funding. A ready project occupies the town’s one active planning slot, and proposals check for suitable owned land before opening.

The live Three.js planning view displays one-unit grid lines, protected surfaces, and a full-scale translucent building model colored by placement validity. It supports map taps, one-square nudges, four facings, camera pan/zoom, district shortcuts, current-building comparison, dimensions/area changes, and a front-door marker. Any resident can preview; only the mayor can confirm a ready project. The server independently checks land ownership, river/bridge, street, home/garden, park, communal garden, work-site, landmark, and doorway constraints. Positions are free within suitable owned land, independent of preset parcel centers.

Institution identity and permanent branch survive relocation. New positions and rotations drive rendering, entrances, pathfinding, collision, minimap footprints, market stall orientation, and relocation of existing reading-court dressing. Migration 0007 adds nullable positions with legacy parcel fallback and includes `ready` in the active-plan uniqueness constraint. Existing placed buildings keep their saved sites.

The progression UI now has a starting institution, three connected first paths, and a clearly marked one-of-two final fork beneath each. Available next choices, the chosen route, current building, and permanently excluded alternatives are labeled explicitly. The library, town center, and harbor choose independently. Larger upgrades can move to a new site during placement instead of requiring a separate relocation first.

Validation: SQL planning tests cover funding without placement, pre-approval/unfunded/mayor checks, protected sites, coordinate/rotation validation, duplicate placement, transaction rollback, no extra spending, and preserved branches. `tests/placement-layout.mjs` checks all 27 models in all four facings, rotated entrances and footprints, protected homes/land, preview purity, and legacy locations. The built Worker planning API test checks the complete shared multiplayer flow.
