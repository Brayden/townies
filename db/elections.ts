...
type Voter={id:string;town_id:string;home:number|null;job:string|null;town_joined_at?:number};
export async function readElection(d:D1Database,r:Voter,now=Date.now()):Promise<ElectionState>{
 ...
 const [candidates,vote,mayor]=await Promise.all([
  d.prepare(`SELECT c.resident_id AS id,r.name,CASE WHEN r.town_joined_at>0 THEN r.town_joined_at ELSE r.created END AS joinedAt,r.town_joined_at AS movedAt,r.xp AS overallXp,c.platform,c.tax,(SELECT COUNT(*) FROM town_projects p WHERE p.town_id=c.town_id AND p.mayor_id=c.resident_id AND p.completed>0) AS delivered,${sums},
   ...
   WHERE c.town_id=? AND c.cycle=? AND c.withdrawn_at=0 GROUP BY c.resident_id ORDER BY c.nominated_at,c.resident_id`)
 ...
 const people=candidates.results.map(c=>{
  ...
  return {id:c.id,name:c.name,joinedAt:c.joinedAt,platform:c.platform,tax:c.tax,delivered:c.delivered,contributions:{month:totals('month'),overall:{...totals('total'),xp:c.movedAt?totals('total').xp:c.overallXp}}};
 });
 return {...schedule,participationLocked:(r.town_joined_at??0)>=schedule.opensAt,contributionMonth:month,mayor,candidates:people,myVote:vote?.candidate_id??null,nominated:candidates.results.some(c=>c.id===r.id)};
}
export async function electionAction(d:D1Database,r:Voter,action:'nominate'|'vote'|'platform',cycle:unknown,candidate:unknown,now=Date.now(),platform?:unknown,tax?:unknown){
 ...
 if((r.town_joined_at??0)>=schedule.opensAt)return {error:'You arrived after voting opened. You can take part in the next election.',status:409};
 ...
 if(action==='platform'){
  ...
  const result=await d.prepare('UPDATE election_candidates SET platform=?,tax=? WHERE town_id=? AND cycle=? AND resident_id=? AND withdrawn_at=0').bind(platform,tax,r.town_id,schedule.cycle,r.id).run();
  return result.meta.changes?null:{error:'Join the ballot before updating your campaign.',status:400};
 }
 if(action==='nominate'){
  if(await d.prepare('SELECT 1 FROM election_candidates WHERE town_id=? AND cycle=? AND resident_id=? AND withdrawn_at>0').bind(r.town_id,schedule.cycle,r.id).first())return {error:'Moving ended this campaign. You can run again next election.',status:409};
...
export async function readMayor(d:D1Database,townId:string,now=Date.now()){
 // Rank the closed ballot before checking residency. A departure leaves the seat
 // vacant, rather than silently electing the runner-up or reviving an older mayor.
 return d.prepare(`WITH winner AS (SELECT c.resident_id,c.town_id,c.cycle,c.platform,c.tax,c.withdrawn_at FROM election_candidates c JOIN election_votes v ON v.town_id=c.town_id AND v.cycle=c.cycle AND v.candidate_id=c.resident_id WHERE c.town_id=? AND c.cycle<=? AND (c.withdrawn_at=0 OR c.withdrawn_at>=CAST(strftime('%s',MIN(date(c.cycle||'-01','+1 month'),date(c.cycle||'-01','+30 days'))) AS INTEGER)*1000) GROUP BY c.cycle,c.resident_id ORDER BY c.cycle DESC,COUNT(*) DESC,c.nominated_at ASC,c.resident_id ASC LIMIT 1) SELECT w.resident_id AS id,r.name,w.cycle,w.platform,w.tax FROM winner w JOIN residents r ON r.id=w.resident_id AND r.town_id=w.town_id WHERE w.withdrawn_at=0`).bind(townId,electionWindow(now).lastClosedCycle).first<{id:string;name:string;cycle:string;platform:string|null;tax:number}>();
}
