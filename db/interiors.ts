import {leaveVisit,type SocialResident} from './social.ts';
...
type Resident={id:string;town_id:string;town_joined_at:number;home:number|null;house:string;x:number;z:number;inside:number;visit_host?:string|null;interior:string;interior_revision:number};
export async function interiorAction(d:D1Database,r:Resident,b:Record<string,unknown>,now:number){
 ...
 if(b.action==='home-enter'){
  ...
  const result=await d.prepare('UPDATE residents SET inside=1,visit_host=NULL,indoor_x=0,indoor_z=2.5,indoor_level=0,interior_revision=interior_revision+1,mowing=0,shift=NULL,riding=0,action_target=NULL,action_started=0,emote=NULL,emote_until=0,seen=? WHERE id=? AND town_id=? AND town_joined_at=? AND home=? AND inside=0 AND (x-?)*(x-?)+(z-?)*(z-?)<=3.61').bind(now,r.id,r.town_id,r.town_joined_at,r.home,door.x,door.x,door.z,door.z).run();
  return result.meta.changes?null:fail('Your home or position changed. Try the door again.');
 }
 if(b.action==='home-exit'){if(r.visit_host){await leaveVisit(d,r as unknown as SocialResident);return null;}
 ...
 if(r.visit_host)return fail('Only the homeowner can decorate this home.',403);
