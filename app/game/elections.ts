...
export type ElectionState=ReturnType<typeof electionWindow>&{mayor:{id:string;name:string;cycle:string;platform:string|null;tax:number}|null;participationLocked?:boolean;contributionMonth:string;candidates:{id:string;name:string;joinedAt:number;platform:string|null;tax:number;delivered:number;contributions:{month:ContributionTotals;overall:ContributionTotals}}[];myVote:string|null;nominated:boolean};
