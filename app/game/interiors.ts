...
export type PlacedFurniture={id:string;x:number;z:number;rotation:number};
export type Interior={wall:string;floor:string;owned:string[];placed:PlacedFurniture[]};
...
export function placementIssue(p:PlacedFurniture,placed:PlacedFurniture[],house?:string){
 ...
 if(f.kind!=='rug'&&Math.abs(a.x)<a.w/2+1&&a.z+a.d/2>size.depth/2-1.5)return 'Keep the doorway clear.';
 if(placed.some(other=>{if(other.id===p.id)return false;const of=furnitureById(other.id);if(!of||((f.kind==='rug')!==(of.kind==='rug')))return false;const b=furnitureRect(other);return Math.abs(a.x-b.x)<(a.w+b.w)/2-.02&&Math.abs(a.z-b.z)<(a.d+b.d)/2-.02;}))return 'Leave some room around the other furniture.';
 return null;
}
export function fittedFurniture(interior:Interior,house?:string){const result:PlacedFurniture[]=[];for(const p of interior.placed)if(!placementIssue(p,result,house))result.push(p);return result;}
export function firstFurnitureSpot(id:string,placed:PlacedFurniture[],house?:string):PlacedFurniture|null{const {width,depth}=roomSize(house);for(let z=-depth/2+.5;z<depth/2;z+=.5)for(let x=-width/2+.5;x<width/2;x+=.5){const p={id,x,z,rotation:0};if(!placementIssue(p,placed,house))return p;}return null;}
