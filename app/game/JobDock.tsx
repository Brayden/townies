...
type Props={chat?:ReactNode;resident:Resident;busy:boolean;patches:number;coins:number;completed:number;waterProgress:number;onAction:()=>void;canAction:boolean;onFindNext:()=>void;onVisitStation:()=>void;onFinish:()=>void};
...
export default function JobDock({chat,resident,busy,patches,coins,completed,waterProgress,onAction,canAction,onFindNext,onVisitStation,onFinish}:Props){
 ...
 return <nav className={`hud bottom-dock paper job-dock ${mowing?'mowing-dock':''} ${chat?'has-chat':''}`} aria-label="Job shift controls">
