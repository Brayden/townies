...
export default function TownHeader({data,active,onElection,onSettings,onTown,actions}:Props){
 ...
 return <header className="hud topbar">
  ...
  <div className="top-right">{actions}<div className="coin-chip paper" aria-label={`${data?.resident.coins??150} coins`}><Coins size={18}/><span>{(data?.resident.coins??150).toLocaleString('en-US')}</span></div><button className="icon-button" aria-label="Settings and controls" onClick={onSettings}><Settings/></button></div>
 </header>;
}
