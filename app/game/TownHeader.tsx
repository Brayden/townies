...
type Props={actions?:ReactNode;data:TownState|null;active:boolean;chatOpen:boolean;onChat:()=>void;onElection:()=>void;onTown:()=>void;onSettings:()=>void};
export default function TownHeader({data,active,chatOpen,onChat,onElection,onSettings,onTown,actions}:Props){
 ...
 return <header className="hud topbar">
  ...
  <div className="hud-tools"><div className="top-right">{actions}<div className="coin-chip paper" aria-label={`${data?.resident.coins??150} coins`}><Coins size={18}/><span>{(data?.resident.coins??150).toLocaleString('en-US')}</span></div><button className="icon-button" aria-label="Settings and controls" onClick={onSettings}><Settings/></button></div>
   {active&&<button type="button" className="town-chat-launch" aria-expanded={chatOpen} aria-controls="town-chat" onClick={onChat} title="Talk with your town"><MessageCircle size={21} aria-hidden="true"/><span>Town chat</span></button>}
  </div>
 </header>;
}
