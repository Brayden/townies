import {COMMUNITY_BUILDINGS,communityBuilding} from './communityBuildings.ts';
import {SQUARE_LOTS,SQUARE_FRONTAGES,hasCommunitySquare} from './communitySquare.ts';
import {TOWN_FARM,FARM_BARN,FARM_ROADS,FARM_SOLIDS,farmIsOpen} from './townFarm.ts';
...
export const REDEVELOPMENTS=COMMUNITY_BUILDINGS;
...
export type PlanningState={squareVersion?:number;farmFunded?:number;territories:string[];institutions:Institution[];buildings:ParcelBuilding[];proposals:PlanProposal[]};
...
export function plotOwned(plot:typeof PLOTS[number],s:PlanningState){return plot.territory==='square'?hasCommunitySquare(s):plot.territory==='core'||s.territories.includes(plot.territory);}
...
export function plotOccupied(id:string,s:PlanningState){const p=plotById(id);if(!p)return false;if(s.buildings.some(b=>b.plot===id))return true;return [...s.institutions.map(i=>({...locationOf(i),...oriented(institutionShape(i).width,institutionShape(i).depth,i.rotation)})),...s.buildings.map(b=>({...locationOf(b),...oriented(7,4,b.rotation)}))].some(b=>Math.abs(b.x-p.x)<(b.width+p.width)/2&&Math.abs(b.z-p.z)<(b.depth+p.depth)/2);}
...
export function townBuildings(s:PlanningState):WorldBuilding[]{const fixed=[...BUILDINGS.filter(b=>b.id!=='library').map(b=>{const p=hasCommunitySquare(s)?SQUARE_FRONTAGES[b.id]:null;return p?{...b,...p,...oriented(b.width,b.depth,p.rotation),modelWidth:b.width,modelDepth:b.depth}:b}),...(farmIsOpen(s)?[FARM_BARN]:[])];return [...fixed,...s.institutions.filter(i=>i.id!=='towncenter'||i.node!=='root').map(i=>{const p=locationOf(i),n=institutionShape(i);return {id:i.id,...p,...oriented(n.width,n.depth,p.rotation),modelWidth:n.width,modelDepth:n.depth,height:n.height,color:n.color,roof:n.roof,name:n.name,action:i.id==='library'?'library':'town'};}),...s.buildings.filter(b=>b.kind!=='garden').map(b=>{const p=locationOf(b),parcel=plotById(b.plot),width=Math.min(7,(parcel?.width??9)-(parcel?.territory==='square'?1:2)),depth=Math.min(4,(parcel?.depth??6)-2);return {id:`site-${b.plot}`,...p,name:communityBuilding(b.kind)?.name??'Community building',...oriented(width,depth,p.rotation),modelWidth:width,modelDepth:depth,height:3.4,color:communityBuilding(b.kind)?.color??'#e5d2ad',roof:communityBuilding(b.kind)?.roof??'#719088',action:b.kind==='pets'?'pets':'community'};})];}
export function townLayout(s:PlanningState):TownLayout{return {bounds:[{x:0,z:-1.5,width:120,depth:113},...(farmIsOpen(s)?[TOWN_FARM]:[]),...TERRITORIES.filter(t=>s.territories.includes(t.id))],buildings:townBuildings(s),solidProps:[...SOLID_PROPS.filter(p=>p.x!==-30||p.z!==47),...(farmIsOpen(s)?FARM_SOLIDS:[])],stalls:s.institutions.find(i=>i.id==='towncenter')?.node==='root'?marketStalls(s):[]};}
export function marketStalls(s:PlanningState){const p=locationOf(s.institutions.find(i=>i.id==='towncenter')!),a=p.rotation*Math.PI/180;return STALLS.filter(stall=>!hasCommunitySquare(s)||stall.x!==0).map(stall=>({...stall,x:p.x+stall.x*Math.cos(a)+(stall.z-14)*Math.sin(a),z:p.z-stall.x*Math.sin(a)+(stall.z-14)*Math.cos(a),rotation:p.rotation}));}
...
export function expansionRoads(s:PlanningState){return [...(farmIsOpen(s)?FARM_ROADS:[]),
