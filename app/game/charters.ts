...
export type PlanningState={farmFunded?:number;territories:string[];institutions:Institution[];buildings:ParcelBuilding[];proposals:PlanProposal[]};
...
export function townBuildings(s:PlanningState):WorldBuilding[]{const fixed=[...BUILDINGS.filter(b=>b.id!=='library'),...(farmIsOpen(s)?[FARM_BARN]:[])];return [...fixed,...s.institutions.filter(i=>i.id!=='towncenter'||i.node!=='root').map(i=>{const p=locationOf(i),n=institutionShape(i);return {id:i.id,...p,...oriented(n.width,n.depth,p.rotation),modelWidth:n.width,modelDepth:n.depth,height:n.height,color:n.color,roof:n.roof,name:n.name,action:i.id==='library'?'library':'town'};}),...s.buildings.filter(b=>b.kind!=='garden').map(b=>{const p=locationOf(b),parcel=plotById(b.plot),width=Math.min(7,(parcel?.width??9)-2),depth=Math.min(4,(parcel?.depth??6)-2);return {id:`site-${b.plot}`,...p,name:REDEVELOPMENTS.find(r=>r.id===b.kind)!.name,...oriented(width,depth,p.rotation),modelWidth:width,modelDepth:depth,height:3.4,color:'#e5d2ad',roof:b.kind==='food'?'#bd8267':b.kind==='pets'?'#a57d97':'#719088',action:b.kind==='pets'?'pets':'town'};})];}
