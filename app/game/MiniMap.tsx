...
import {PawPrint,Tractor,LockKeyhole,Package,Check,BookOpen,Sprout,Mailbox,Flower2,Landmark,Shirt,Store,Coffee,GraduationCap,Truck,Droplets,Scissors,Leaf} from 'lucide-react';
...
const maintenanceIcons={sweep:Truck,wash:Droplets,trim:Scissors,rake:Leaf};
const maintenanceLabels={sweep:'Street sweeping',wash:'Sidewalk washing',trim:'Hedge trimming',rake:'Leaf raking'};
export default function MiniMap({data,getPosition,onLook,level=0}:{level?:number;data:TownState;getPosition:()=>{x:number;z:number}|undefined;onLook:(x:number,z:number)=>void}){
 const map=useRef<SVGSVGElement>(null),[pixels,setPixels]=useState(220);
 useEffect(()=>{if(!map.current)return;const observer=new ResizeObserver(([entry])=>setPixels(Math.max(80,entry.contentRect.width)));observer.observe(map.current);return()=>observer.disconnect()},[]);
 ...
 const layout=useMemo(()=>townLayout(data.planning),[data.planning]);
 const maintenanceStops=useMemo(()=>WORK_TARGETS.filter(t=>t.job===data.resident.shift&&!isTownBlocked(t.x,t.z,layout)),[data.resident.shift,layout]);
 const buildings=townBuildings(data.planning),expanded=data.planning.territories.length>0;
 const paper=data.resident.shift==='paper',garden=data.resident.shift==='garden',parcel=data.resident.shift==='deliver',maintenance=isMaintenance(data.resident.shift),route=paper||garden||parcel||maintenance,stops=maintenance?maintenanceStops:paper?paperStops:parcel?WORK_TARGETS.filter(t=>t.job==='deliver'&&data.parcelHomes?.includes(t.home!)):gardenStops,Marker=paper?Mailbox:parcel?Package:Flower2,zoom=MAP_ZOOMS[Math.max(0,Math.min(MAP_ZOOMS.length-1,level))],span=(expanded?236:184)/zoom;
 const clamp=(v:number,min:number,max:number)=>span>=max-min?(min+max)/2:Math.max(min+span/2,Math.min(max-span/2,v));
 const cx=clamp(position.x,-118,expanded?116:64),cz=clamp(position.z,expanded?-99:-64,expanded?101:64),unit=span/220,workUnit=span/pixels;
 const delivered=new Set(data.worldWork.filter(w=>Date.now()-w.completed<WORK_DAY_MS).map(w=>w.id));
 const workMarkers=maintenance?minimapWorkMarkers(stops,delivered,{x:cx,z:cz,span,pixels}):[],WorkIcon=isMaintenance(data.resident.shift)?maintenanceIcons[data.resident.shift]:Droplets;
 const workLabel=isMaintenance(data.resident.shift)?maintenanceLabels[data.resident.shift]:'';
 ...
 return <section className="town-minimap" aria-label={maintenance?'Maintenance job minimap':paper?'Paper delivery minimap':garden?'Community gardening minimap':parcel?'Parcel delivery minimap':'Town minimap'} onKeyDown={e=>e.stopPropagation()}>
   <svg ref={map} className="town-minimap-map" onClick={e=>lookFromMap(e.currentTarget,e.clientX,e.clientY)} style={{cursor:'crosshair'}} aria-describedby="minimap-look-hint" viewBox={`${cx-span/2} ${cz-span/2} ${span} ${span}`} role="img" aria-label={maintenance?`${count} of ${stops.length} areas completed. Gold tool markers need work; green checks are tidy. Numbers show nearby tasks remaining. Zoom in to separate work areas.`:paper?`${count} of ${stops.length} houses delivered. Green checked mailboxes are delivered; gray mailboxes are waiting.`:garden?`${count} of ${stops.length} beds watered. Green checked flowers are watered; gray flowers need water.`:'Town streets, homes, neighbors, and your position.'}>
    ...
    {maintenance&&stops.map(t=>{const done=delivered.has(t.group);return <rect key={t.id} x={t.x-(t.width??1)/2} y={t.z-(t.depth??1)/2} width={t.width??1} height={t.depth??1} fill={done?'#368b52':'#e1a736'} opacity={done?.3:.6}><title>{`${t.title}: ${done?'Completed':'Needs work'}`}</title></rect>})}
    {workMarkers.map(pin=>{const size=16*workUnit,t=pin.target,label=`${workLabel}: ${pin.done?'Completed':`${pin.pending} ${pin.pending===1?'spot':'spots'} need work`}. Look at ${t.title.toLowerCase()}.`;return <g key={pin.id} className="minimap-work-pin" role="button" tabIndex={0} aria-label={label} onClick={e=>{e.stopPropagation();onLook(t.x,t.z)}} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();onLook(t.x,t.z)}}}>
     <title>{label}</title><rect x={t.x-size/2-2*workUnit} y={t.z-size/2-2*workUnit} width={size+4*workUnit} height={size+4*workUnit} rx={5*workUnit} fill={pin.done?'#e1f2d7':'#fff3cf'} stroke={pin.done?'#327344':'#96611e'} strokeWidth={1.3*workUnit}/>
     {pin.done?<Check x={t.x-size/2} y={t.z-size/2} width={size} height={size} color="#286c3a" strokeWidth={3}/>:<WorkIcon x={t.x-size/2} y={t.z-size/2} width={size} height={size} color="#925b19" strokeWidth={2.4}/>}
     {!pin.done&&pin.pending>1&&<><circle cx={t.x+9*workUnit} cy={t.z-8*workUnit} r={6.5*workUnit} fill="#976120" stroke="#fff5d9" strokeWidth={workUnit}/><text x={t.x+9*workUnit} y={t.z-7.5*workUnit} textAnchor="middle" dominantBaseline="central" fontSize={9*workUnit} fontWeight="800" fill="#fffbed">{pin.pending>99?'99+':pin.pending}</text></>}
    </g>})}
    ...
    <circle cx={position.x} cy={position.z} r={4.5*(maintenance?workUnit:unit)} fill="#fffbea" stroke="#244b67" strokeWidth={2*unit}><title>You</title></circle>
    <circle cx={position.x} cy={position.z} r={1.5*(maintenance?workUnit:unit)} fill="#244b67"/>
   </svg>
   <span className="sr-only" id="minimap-look-hint">Click or tap the map to look around. Hover the map or focus its controls for quick zoom. On touch screens, use the plus and minus buttons above the map.</span>
   {maintenance&&<><span className="minimap-work-heading" title={`${workLabel} · ${count} of ${stops.length} complete`}><WorkIcon size={14}/>{stops.length-count} left</span><span className="minimap-work-legend"><i/>To do <Check size={12}/>Done</span></>}
   {route&&!maintenance&&<span className="minimap-route-count" title={maintenance?'Green: tidy. Gray: needs work.':paper||parcel?'Green: delivered. Gray: waiting.':'Green: watered. Gray: needs water.'}>{count}/{stops.length}</span>}
 </section>;
}
