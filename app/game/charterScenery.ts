...
import {TERRITORIES,PLOTS,FERRY_STOPS,plotOwned,plotOccupied,plotById,nodeById,institutionShape,townBuildings,expansionRoads,type PlanningState} from './charters.ts';
...
export function charterScenery(k:Kit,s:PlanningState){const {box,ball,cylinder,roof,bench,flower}=k,root=new THREE.Group();
 ...
 function landmark(id:string,b:ReturnType<typeof townBuildings>[number],style:string){const g=new THREE.Group();g.position.set(b.x,0,b.z);root.add(g);const w=b.width,d=b.depth,h=b.height;
 ...
 for(const i of s.institutions){if(i.node==='root'&&i.id!=='harbor')continue;const b=buildings.find(b=>b.id===i.id)!;landmark(i.id,b,nodeById(i.node)?.style??'harbor');}
 for(const building of s.buildings){const p=plotById(building.plot)!;if(building.kind==='garden'){for(const x of [-2,2])bench(p.x+x,p.z,root);for(let i=0;i<20;i++)flower(p.x-3+i%10*.65,p.z-2+Math.floor(i/10)*3,'#e5b2bd',root);cylinder(1,.25,'#d5c19a',p.x,.35,p.z,root);}else landmark(building.plot,buildings.find(b=>b.id===`site-${building.plot}`)!,building.kind==='food'?'market':'workshop');}
