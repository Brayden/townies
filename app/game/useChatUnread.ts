    ...
    const load=async()=>{
      ...
      try{
        const values=await Promise.all(channels.map(async channel=>{
          const cursor=cursors.current[channel],signature=JSON.stringify(cursor);
          const params=new URLSearchParams({town:town!,channel,summary:'1',after:String(cursor.created),seen:cursor.ids.join(',')});
          ...
          return {channel,signature,count:Math.max(0,Number(value.unread)||0)};
        }));
        if(!alive)return;
        const counts:Record<string,number>={};
        for(const value of values){if(value.signature!==JSON.stringify(cursors.current[value.channel])){dirty=true;continue}counts[value.channel]=value.count}
