...
import {MAP_ZOOMS} from './minimapWork';
...
export default function MapControls({data,getPosition,onLook,scene,open,onOpenChange,mode,onMode}:Props){
 ...
 return <div className="map-hud">
  ...
  <div className="minimap-quick-zoom" role="group" aria-label="Quick minimap zoom" onKeyDown={e=>e.stopPropagation()}>
   <button className="icon-button" aria-label="Zoom minimap out" title="Zoom minimap out" disabled={level===0} onClick={()=>setLevel(v=>Math.max(0,v-1))}><Minus size={18}/></button>
   <button className="icon-button" aria-label="Zoom minimap in" title="Zoom minimap in" disabled={level===MAP_ZOOMS.length-1} onClick={()=>setLevel(v=>Math.min(MAP_ZOOMS.length-1,v+1))}><Plus size={18}/></button>
  </div>
  <Popover open={open} onOpenChange={onOpenChange}>
   ...
   <PopoverContent className="hud-popover map-tools" side="top" align="end" sideOffset={8} onKeyDown={e=>e.stopPropagation()}>
    ...
    <div className="map-control-row"><span>Minimap · {MAP_ZOOMS[level]}×</span><button className="icon-button" aria-label="Zoom minimap out" disabled={level===0} onClick={()=>setLevel(v=>Math.max(0,v-1))}><Minus size={18}/></button><button className="icon-button" aria-label="Zoom minimap in" disabled={level===MAP_ZOOMS.length-1} onClick={()=>setLevel(v=>Math.min(MAP_ZOOMS.length-1,v+1))}><Plus size={18}/></button></div>
