...
export default function ElectionPanel({election:e,residentId,busy,onNominate,onPlatform,onVote,civic}:Props){
 ...
 return <div className="election-panel">
  ...
  {e.participationLocked&&<p className="note">You arrived after voting opened. You can meet the candidates now and participate in the next election.</p>}
  {!e.participationLocked&&(!e.nominated||!e.active)&&<details className="campaign-editor" open={!e.nominated}><summary>{e.nominated?'Edit my campaign promises':'Choose my campaign promises'}</summary><p>One flagship project. A clear tax policy. Your project becomes the featured town project if you win. You can edit this plan before voting opens. Once voting begins, the promise is permanently locked for this election.</p><ProjectPicker value={platform} onChange={setPlatform} civic={civic} disabled={busy}/>{!available.length&&<p>Every upgrade is complete. Run to steward the town and set its tax policy.</p>}<h3>My tax policy</h3><TaxPicker value={tax} onChange={setTax} disabled={busy}/><button className="primary-button full" disabled={busy||available.length>0&&!platform} onClick={()=>e.nominated?onPlatform(platform||null,tax):onNominate(platform||null,tax)}><UserRoundPlus size={17}/>{e.nominated?'Save my promises':'Join the ballot with this plan'}</button></details>}
  {e.nominated&&e.active&&<p className="promise-delivered"><Check size={16}/>You’re on the ballot. Campaign promises are permanently locked for this election.</p>}
