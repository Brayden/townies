...
import type {WorldBuilding} from './charters';
...
export function civicScenery(k:Kit,buildings:WorldBuilding[]=BUILDINGS,square=false){
 ...
 for(const p of CIVIC_PROJECTS){const g=new THREE.Group();g.name=p.id;g.visible=false;groups.set(p.id,g);
  switch(p.id){
   ...
   case 'market-canopies':for(const s of STALLS.filter(stall=>!square||stall.x!==0)){roof(3.4,2.6,.65,'#dfb16d',s.x,3.3,s.z,g);for(const dx of [-1.5,1.5]){box(.07,1.6,.07,'#a1845c',s.x+dx,3,s.z,g);banner(s.x+dx,3.4,s.z,g)}}break;
   case 'shopfronts':for(const footprint of buildings.filter(b=>['general','clothing','cafe'].includes(b.id))){const b={...footprint,width:footprint.modelWidth??footprint.width,depth:footprint.modelDepth??footprint.depth};const shop=new THREE.Group();shop.position.set(b.x,0,b.z);shop.rotation.y=(b.rotation??0)*Math.PI/180;g.add(shop);for(const dx of [-b.width*.32,b.width*.32]){box(1.65,.3,.5,'#bc8e63',dx,1.02,b.depth/2+.35,shop);for(let i=0;i<5;i++)ball(.18,i%2?'#efaab4':'#edd48a',dx-.55+i*.27,1.27,b.depth/2+.35,shop)}for(let n=0;n<8;n++)box(b.width/8,.12,1.45,n%2?'#faf0ce':'#739e90',-b.width/2+(n+.5)*b.width/8,3.08,b.depth/2+.65,shop)}break;
