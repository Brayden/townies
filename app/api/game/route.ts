...
if(r.inside&&b.action==='heartbeat'){await d.prepare('UPDATE residents SET seen=? WHERE id=?').bind(now,r.id).run();r=(await d.prepare('SELECT * FROM residents WHERE id=?').bind(r.id).first<Row>())!;return json(await state(r));}
if(r.inside&&!['home-enter','home-exit','home-place','home-store','home-finish'].includes(b.action))return json({error:'Step outside to take part in town activities.'},409);
...
if(['home-enter','home-exit','home-place','home-store','home-finish'].includes(b.action)){const error=await interiorAction(d,r,b,now);if(error)return json(error,error.status);}
 ...
 const moved=await d.prepare('UPDATE residents SET emote=CASE WHEN ? THEN NULL ELSE emote END,emote_until=CASE WHEN ? THEN 0 ELSE emote_until END,x=?,z=?,seen=? WHERE id=? AND town_id=? AND town_joined_at=? AND inside=0 AND seen=? AND x=? AND z=? RETURNING *').bind(standing?1:0,standing?1:0,allowed?x:r.x,allowed?z:r.z,now,r.id,r.town_id,r.town_joined_at,r.seen,r.x,r.z).first<Row>();
