...
type Row={cat_pet:string|null;dog_pet:string|null;town_joined_at:number;house:string;upkeep_due:number;upkeep_note:string;riding:number;outfit:string;hat:string;accessory:string;planning?:PlanningState;id:string;token_hash:string;town_id:string;name:string;color:string;home:number|null;job:string|null;coins:number;xp:number;education:number;last_study:string|null;x:number;z:number;items:string;shift:FieldJob|null;papers:number;parcels:number;water:number;bag:number;action_target:string|null;action_started:number;mowing:number;seen:number;created:number};
...
if(b.action==='pet'){const error=await petAction(d,r,b);if(error)return json(error,error.status);}
