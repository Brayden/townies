...
type Row={planning?:PlanningState;id:string;token_hash:string;town_id:string;name:string;color:string;home:number|null;job:string|null;coins:number;xp:number;education:number;last_study:string|null;x:number;z:number;items:string;shift:FieldJob|null;papers:number;parcels:number;water:number;bag:number;action_target:string|null;action_started:number;mowing:number;seen:number;created:number};
...
async function auth(req:Request){const id=req.headers.get('oai-authenticated-user-id');if(!id)return null;const r=await db().prepare('SELECT * FROM residents WHERE token_hash=?').bind(await hash(id)).first<Row>();if(r){r.planning=await readPlanning(db(),r.town_id,r.id);}
...
if(['plan-propose','plan-vote','plan-fund','plan-place'].includes(b.action)){const error=await planningAction(d,r,b,now);if(error)return json({error:error.error},error.status);}
else if(b.action==='ferry'){const planning=r.planning??await readPlanning(d,r.town_id,r.id,now);if(!planning.territories.includes('island'))return json({error:'The town must approve and fund Sunrise Island first.'},409);const from=FERRY_STOPS.findIndex(p=>Math.hypot(p.x-r!.x,p.z-r!.z)<=2);if(from<0)return json({error:'Visit a ferry landing to board.'},400);const to=FERRY_STOPS[1-from];await d.prepare('UPDATE residents SET x=?,z=?,seen=?,shift=NULL,mowing=0,action_target=NULL,action_started=0 WHERE id=?').bind(to.x,to.z,now,r.id).run();}
