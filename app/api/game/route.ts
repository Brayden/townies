...
type Row={id:string;token_hash:string;town_id:string;name:string;color:string;home:number|null;job:string|null;coins:number;xp:number;education:number;last_study:string|null;x:number;z:number;items:string;shift:FieldJob|null;papers:number;parcels:number;water:number;bag:number;action_target:string|null;action_started:number;mowing:number;seen:number;created:number};
...
async function auth(req:Request){const id=req.headers.get('oai-authenticated-user-id');if(!id)return null;const r=await db().prepare('SELECT * FROM residents WHERE token_hash=?').bind(await hash(id)).first<Row>();if(r&&isTownBlocked(r.x,r.z)){const p=safeTownPosition(r.x,r.z);await db().prepare('UPDATE residents SET x=?,z=? WHERE id=? AND x=? AND z=?').bind(p.x,p.z,r.id,r.x,r.z).run();return await db().prepare('SELECT * FROM residents WHERE id=?').bind(r.id).first<Row>()}return r}
async function state(r:Row){const d=db();const now=Date.now();const [town,people,completed,history,grass,worldWork,election]=await Promise.all([d.prepare('SELECT * FROM towns WHERE id=?').bind(r.town_id).first<any>(),d.prepare('SELECT id,name,color,home,items,x,z,seen,mowing,shift,papers,parcels,water,bag,action_target,action_started FROM residents WHERE town_id=?').bind(r.town_id).all<any>(),d.prepare('SELECT task FROM work WHERE town_id=? AND completed>?').bind(r.town_id,now-120000).all<{task:string}>(),d.prepare('SELECT name,text FROM events WHERE town_id=? ORDER BY created DESC LIMIT 6').bind(r.town_id).all(),d.prepare('SELECT cell FROM lawn_cells WHERE town_id=? AND cut_at>?').bind(r.town_id,now-GRASS_REGROW_MS).all<{cell:string}>(),d.prepare('SELECT object_id AS id,target_id AS target,completed FROM world_work WHERE town_id=? AND completed>?').bind(r.town_id,now-WORK_DAY_MS).all(),readElection(d,r,now)]);return {election,resident:{id:r.id,name:r.name,color:r.color,home:r.home,job:r.job,coins:r.coins,xp:r.xp,education:r.education,lastStudy:r.last_study,x:r.x,z:r.z,items:JSON.parse(r.items),mowing:!!r.mowing,shift:r.shift,papers:r.papers,parcels:r.parcels,water:r.water,bag:r.bag,wateringTarget:r.action_target,wateringStarted:r.action_started},town:{id:town.id,name:town.name,private:!!town.private,key:town.invite??undefined,treasury:town.treasury,project:town.project,prosperity:Math.min(100,50+Math.floor(town.project/4)+completed.results.length*2+Math.floor(grass.results.length/8)+Math.floor(worldWork.results.length/6)),residents:people.results.length},peers:people.results.filter(p=>p.seen>now-12000).map(({seen,home,items,action_target,action_started,...p})=>({...p,wateringTarget:action_target,wateringStarted:action_started})),properties:people.results.filter(p=>p.home!==null).map(p=>({home:p.home,items:JSON.parse(p.items),name:p.name})),occupied:people.results.filter(p=>p.home!==null).map(p=>p.home),completed:completed.results.map(t=>t.task),worldWork:worldWork.results,lawnCuts:grass.results.map(c=>c.cell),events:history.results}}
...
if(b.action==='setup'){const job=JOBS.find(j=>j.id===b.job);const h=HOMES.find(h=>h.id===b.home);if(!job||!h)return json({error:'Choose one of the available jobs and homes.'},400);if(r.home!==null)return json(await state(r));try{await d.prepare('UPDATE residents SET job=?,home=? WHERE id=? AND home IS NULL').bind(job.id,h.id,r.id).run()}catch{return json({error:'A neighbor just chose that home. Please choose another.'},409)}await d.prepare('INSERT INTO events(id,town_id,name,text,created) VALUES(?,?,?,?,?)').bind(crypto.randomUUID(),r.town_id,r.name,`moved into ${h.name}`,now).run()}
else if(b.action==='nominate'||b.action==='vote'){
 const error=await electionAction(d,r,b.action,b.cycle,b.candidate,now);if(error)return json({error:error.error},error.status);
}
 ...
 await d.prepare('UPDATE residents SET mowing=?,shift=NULL,action_target=NULL,action_started=0 WHERE id=?').bind(b.active===true?1:0,r.id).run();
}
 ...
 await d.prepare('UPDATE residents SET shift=?,mowing=0,action_target=NULL,action_started=0 WHERE id=?').bind(shift,r.id).run();
}
 ...
 const column={paper:'papers',deliver:'parcels',garden:'water',clean:'bag'}[r.shift],amount=r.shift==='clean'?0:CAPACITY[r.shift];
 ...
 if(target.job==='clean'?r.bag>=CAPACITY.clean:r[field as 'papers'|'parcels'|'water']<1)return json({error:target.job==='clean'?'Your bag is full. Empty it at the recycling station.':'You are out of supplies. Visit the supply stand or fountain to refill.'},409);
 if(b.action==='water-start'){
  ...
  const pay=WORK_PAY[target.job]-(r.job===target.job?0:1),xp=target.job==='garden'?3:2;
  const supplyCheck=target.job==='clean'?'bag<8':`${field}>0`;
  const waterCheck=target.job==='garden'?`AND action_target=? AND action_started=?`:'';
  const claim=d.prepare(`INSERT INTO world_work(key,town_id,object_id,target_id,completed) SELECT ?,?,?,?,? WHERE EXISTS(SELECT 1 FROM residents WHERE id=? AND shift=? AND mowing=0 AND ${supplyCheck} AND (x-?)*(x-?)+(z-?)*(z-?)<=4 ${waterCheck}) ON CONFLICT(key) DO UPDATE SET target_id=excluded.target_id,completed=excluded.completed WHERE world_work.completed<?`).bind(`${r.town_id}:${target.group}`,r.town_id,target.group,target.id,now,r.id,target.job,target.x,target.x,target.z,target.z,...(target.job==='garden'?[target.id,r.action_started]:[]),now-WORK_DAY_MS);
  const results=await d.batch([claim,d.prepare(`UPDATE residents SET coins=coins+?,xp=xp+?,${field}=${field}${target.job==='clean'?'+1':'-1'},action_target=NULL,action_started=0 WHERE id=? AND changes()=1`).bind(pay,xp,r.id),d.prepare('UPDATE towns SET treasury=treasury+1 WHERE id=? AND changes()=1').bind(r.town_id)]);
  ...
  return json({...await state(r),workReward:{coins:pay,xp,target:target.id,job:target.job}});
 }
}
else if(b.action==='heartbeat'){
 const x=Number(b.x),z=Number(b.z);
 if(!Number.isFinite(x)||!Number.isFinite(z)||Math.abs(x)>60||z< -58||z>55)return json({error:'Stay inside the town boundary.'},400);
 const elapsed=Math.max(.1,Math.min((now-r.seen)/1000,3));
 const blocked=isTownBlocked;
 const trail=Array.isArray(b.path)?b.path:[];
 if(trail.length>32||trail.some((p:any)=>!p||typeof p.x!=='number'||typeof p.z!=='number'||!Number.isFinite(p.x)||!Number.isFinite(p.z)||Math.abs(p.x)>60||p.z< -58||p.z>55))return json({error:'That movement path is not valid.'},400);
 ...
 const moved=await d.prepare('UPDATE residents SET x=?,z=?,seen=? WHERE id=? AND seen=? AND x=? AND z=? RETURNING *').bind(allowed?x:r.x,allowed?z:r.z,now,r.id,r.seen,r.x,r.z).first<Row>();
 let cut=0;
 if(moved&&allowed&&distance>.05&&r.mowing&&now-r.seen<4000){
  const patches=[...new Map(segments.flatMap(({a,b})=>sweptGrass(a.x,a.z,b.x,b.z)).map(c=>[c.id,c])).values()];
  if(patches.length){const pay=r.job==='mow'?2:1;const statements=patches.flatMap(c=>[
   ...
   d.prepare('UPDATE towns SET treasury=treasury+1 WHERE id=? AND changes()=1').bind(r!.town_id)
  ]);const results=await d.batch(statements);cut=results.filter((_,i)=>i%3===0).reduce((n,v)=>n+v.meta.changes,0);}
 }
 r=(await d.prepare('SELECT * FROM residents WHERE id=?').bind(r.id).first<Row>())!;
 return json({...await state(r),corrected:!allowed||!moved,mowReward:cut?{patches:cut,coins:cut*(r.job==='mow'?2:1)}:null});
}
...
else if(b.action==='job'){if(!JOBS.some(j=>j.id===b.job))return json({error:'Choose a job on the board.'},400);await d.prepare('UPDATE residents SET job=?,mowing=0,shift=NULL,action_target=NULL,action_started=0 WHERE id=?').bind(b.job,r.id).run()}
