...
if(['life-emote','life-picnic','life-basket','life-plant','life-water','life-harvest'].includes(b.action)){const error=await sharedLifeAction(d,r,b,now);if(error)return json(error,error.status);}
 ...
 const moved=await d.prepare('UPDATE residents SET emote=CASE WHEN ? THEN NULL ELSE emote END,emote_until=CASE WHEN ? THEN 0 ELSE emote_until END,x=?,z=?,seen=? WHERE id=? AND town_id=? AND town_joined_at=? AND seen=? AND x=? AND z=? RETURNING *').bind(standing?1:0,standing?1:0,allowed?x:r.x,allowed?z:r.z,now,r.id,r.town_id,r.town_joined_at,r.seen,r.x,r.z).first<Row>();
