...
type Row={town_joined_at:number;house:string;upkeep_due:number;upkeep_note:string;riding:number;outfit:string;hat:string;accessory:string;planning?:PlanningState;id:string;token_hash:string;town_id:string;name:string;color:string;home:number|null;job:string|null;coins:number;xp:number;education:number;last_study:string|null;x:number;z:number;items:string;shift:FieldJob|null;papers:number;parcels:number;water:number;bag:number;action_target:string|null;action_started:number;mowing:number;seen:number;created:number};
...
if(b.action==='move-town'){const error=await moveTown(d,r,b,now);if(error)return json(error,error.status);}
 ...
 const moved=await d.prepare('UPDATE residents SET x=?,z=?,seen=? WHERE id=? AND town_id=? AND town_joined_at=? AND seen=? AND x=? AND z=? RETURNING *').bind(allowed?x:r.x,allowed?z:r.z,now,r.id,r.town_id,r.town_joined_at,r.seen,r.x,r.z).first<Row>();
