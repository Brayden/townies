...
export class ResidentCoordinator extends DurableObject<Cloudflare.Env>{
 ...
 async revoke(session:string){return this.serial(async()=>{this.ctx.storage.kv.delete('session:'+session);const m=this.ctx.storage.kv.get<Member>('member');if(m)await this.town(m.town).revokeSocketSession(m.town,session)})}
 async revokeAll(){return this.serial(async()=>{const m=this.ctx.storage.kv.get<Member>('member');for(const [key] of this.ctx.storage.kv.list({prefix:'session:'})){this.ctx.storage.kv.delete(key);if(m)await this.town(m.town).revokeSocketSession(m.town,key.slice(8))}})}
 ...
 async socketGrant(session:string,identity:string,url:string){return this.serial(async()=>{const expires=this.ctx.storage.kv.get<number>('session:'+session)??0;if(expires<=Date.now()||this.ctx.storage.kv.get('identity')!==identity)return null;let m=await this.member(identity);if(!m)return null;if(this.ctx.storage.kv.get('transfer')){await this.resumeMove(m);m=this.ctx.storage.kv.get<Member>('member')!}const r=await this.town(m.town).resident(m.town,identity);return r?{identity,session,expires,town:m.town,resident:m.id,membership:r.town_joined_at as number,url}:null})}
 ...
 private async social(m:Member,call:Call,b:Record<string,any>){
  ...
  if(call.method==='POST'){
   ...
   if(['request','accept','remove'].includes(b.action)){await this.town(m.town).syncFriends(m.town);if(target&&target.town_id!==m.town)await this.town(target.town_id).syncFriends(target.town_id)}if(b.action!=='read'){await this.town(m.town).notifySocial(m.town,m.id);if(target)await this.town(target.town_id).notifySocial(target.town_id,b.to)}return json({saved:true});
  }
