...
import {townBuildings,townLayout,FERRY_STOPS,nodeById,type PlanningState} from '@/app/game/charters';
...
export async function GET(req:Request){try{const r=await auth(req);if(!r&&!await gameIdentity(req))return json({error:'Sign in to enter your town.'},401);return json(r?await state(r):{resident:null})}catch(e){console.error('town state',e);return json({error:'The town is taking a moment to connect. Please try again.'},503)}}
...
if(b.action==='join'){const account=await gameIdentity(req);if(!account)return json({error:'Sign in to keep your resident and town saved across devices.'},401);if(r)return json(await state(r));const name=String(b.name??'').trim().slice(0,24);if(name.length<2)return json({error:'Please enter a name with at least two letters.'},400);if(hasProfanity(name))return json({error:NAME_LANGUAGE_ERROR},400);if(b.mode==='private'&&hasProfanity(String(b.townName||`${name}’s Hollow`).trim().slice(0,32)))return json({error:TOWN_LANGUAGE_ERROR},400);let townId:string;const mode=b.mode;if(mode==='private'){townId=crypto.randomUUID();const invite=token().slice(0,16).toUpperCase();await d.prepare('INSERT INTO towns(id,name,invite,private,created,square_version) VALUES(?,?,?,?,?,1)').bind(townId,String(b.townName||`${name}’s Hollow`).trim().slice(0,32),invite,1,now).run()}else if(mode==='key'){const town=await d.prepare('SELECT id FROM towns WHERE invite=?').bind(String(b.key??'').trim().toUpperCase()).first<{id:string}>();if(!town)return json({error:'No town has that invitation key. Check it and try again.'},404);townId=town.id}else{await d.prepare("INSERT OR IGNORE INTO towns(id,name,private,created,square_version) VALUES('willowbrook','Willowbrook',0,?,1)").bind(now).run();const town=await d.prepare('SELECT t.id FROM towns t LEFT JOIN residents r ON r.town_id=t.id WHERE t.private=0 GROUP BY t.id HAVING COUNT(r.id)<50 ORDER BY COUNT(r.id) DESC,t.created LIMIT 1').first<{id:string}>();if(town)townId=town.id;else{townId=crypto.randomUUID();await d.prepare('INSERT INTO towns(id,name,private,created,square_version) VALUES(?,?,0,?,1)').bind(townId,`Willowbrook ${now.toString().slice(-4)}`,now).run()}}
